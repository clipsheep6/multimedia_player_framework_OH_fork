...
void PlayerXCollie::TimerCallback(void *data)
{
    ...
    if (threadDeadlockCount_ >= threshold) {
        MEDIA_LOGE("PlayerXCollie process timeout, player process exit.");
        _exit(-1);
    }
}
...
int32_t PlayerXCollie::SetTimer(const std::string &name, bool recovery, uint32_t timeout)
{
    ...
    return HiviewDFX::XCollie::GetInstance().SetTimer(name, timeout, func, this, flag);
#else
    return -1;
#endif
}
...
int32_t PlayerXCollie::SetTimerByLog(const std::string &name, uint32_t timeout)
{
    ...
    return HiviewDFX::XCollie::GetInstance().SetTimer(name, timeout, nullptr, this, flag);
#else
    return -1;
#endif
}
