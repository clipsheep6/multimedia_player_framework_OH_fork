...
int32_t PlayerServerMem::RecoverMemByUser()
{
    if (isReleaseMemByManage_) {
        MEDIA_LOGI("enter");
        PlayerServer::RecoverCodecBuffers();
        int32_t currentTime = 0;
        int32_t ret = PlayerServer::GetCurrentTime(currentTime);
        CHECK_AND_RETURN_RET_LOG(ret == MSERR_OK, ret, "GetCurrentTime Fail");
        ret = PlayerServer::Seek(currentTime, SEEK_CLOSEST);
