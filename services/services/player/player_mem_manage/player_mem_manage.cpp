...
{
    for (auto iter = appPlayerInfo.memRecallVec.begin(); iter != appPlayerInfo.memRecallVec.end(); iter++) {
        ...
        (*iter)(static_cast<int32_t>(MemManageRecallType::TICK_TRIGGER_RECALL_TYPE), 0);
...
int32_t PlayerMemManage::RegisterPlayerServer(int32_t uid, int32_t pid, MemManageRecall memRecall)
{
    std::lock_guard<std::recursive_mutex> lock(recMutex_);
    if (memRecall == nullptr) {
    ...
    appPlayerInfo.memRecallVec.push_back(memRecall);

    return MSERR_OK;
}

void PlayerMemManage::FindDeregisterPlayerFromVec(bool &isFind, AppPlayerInfo &appPlayerInfo,
    MemManageRecall memRecall)
{
    for (auto iter = appPlayerInfo.memRecallVec.begin();
        iter != appPlayerInfo.memRecallVec.end();) {
        if (*iter == memRecall) {
            iter = appPlayerInfo.memRecallVec.erase(iter);
            MEDIA_LOGI("Remove PlayerServerTask from vector size:%{public}lu",
                appPlayerInfo.memRecallVec.size());
            isFind = true;
            break;
        } else {
            iter++;
...
int32_t PlayerMemManage::DeregisterPlayerServer(MemManageRecall memRecall)
{
    std::lock_guard<std::recursive_mutex> lock(recMutex_);
    if (memRecall == nullptr) {
        MEDIA_LOGE("input param invalid");
    ...
    for (auto &[uid, pidPlayersInfo] : playerManage_) {
        for (auto &[pid, appPlayerInfo] : pidPlayersInfo) {
            FindDeregisterPlayerFromVec(isFind, appPlayerInfo, memRecall);
            if (appPlayerInfo.memRecallVec.size() == 0) {
    ...
    if (!isFind) {
        MEDIA_LOGW("Not find memRecall, maybe already deregister");
...
int32_t PlayerMemManage::HandleForceReclaim(int32_t uid, int32_t pid)
{
    ...
    for (auto &[findUid, pidPlayersInfo] : playerManage_) {
        ...
        for (auto &[findPid, appPlayerInfo] : pidPlayersInfo) {
            ...
            for (auto iter = appPlayerInfo.memRecallVec.begin(); iter != appPlayerInfo.memRecallVec.end(); iter++) {
                (*iter)(static_cast<int32_t>(MemManageRecallType::FORCE_RECLAIM_RECALL_TYPE), 0);
                MEDIA_LOGI("call MemManageRecall success");
            }
        }
    }
...
void PlayerMemManage::HandleOnTrimLevelLow()
{
    for (auto &[findUid, pidPlayersInfo] : playerManage_) {
        for (auto &[findPid, appPlayerInfo] : pidPlayersInfo) {
            ...
            for (auto iter = appPlayerInfo.memRecallVec.begin(); iter != appPlayerInfo.memRecallVec.end(); iter++) {
                (*iter)(static_cast<int32_t>(MemManageRecallType::ON_TRIM_RECALL_TYPE),
                    Memory::SystemMemoryLevel::MEMORY_LEVEL_LOW);
                MEDIA_LOGI("call MemManageRecall success");
            }
        }
    }
