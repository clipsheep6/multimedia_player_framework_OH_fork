...
public:
    ...
    bool GetReleaseMemByManage();
    bool IsAudioPlayer();
    void ContinuousNotPlayingCntAdd();
    void SetContinuousNotPlayingCntZero();
    int32_t GetContinuousNotPlayingCnt();

private:
    struct RecoverConfigInfo {
        bool isReleaseMemByManage = false;
        bool isRecoverMemByUser = false;
    ...
    bool isCloseMemoryRecycle_ = false;
    ...
    void SetReleaseMemByManage(bool value);
    bool GetRecoverMemByUser();
    void SetRecoverMemByUser(bool value);
...
public:
    BaseState(PlayerServerTask &serverTask, const std::string &name)
        : PlayerServerState(name), serverTask_(serverTask) {}
