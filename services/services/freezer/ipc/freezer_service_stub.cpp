...
FreezerServiceStub::FreezerServiceStub()
{
    ...
    freezerServer_ = std::make_shared<FreezerServer>();
...
{
    CHECK_AND_RETURN_RET_LOG(freezerServer_ != nullptr, MSERR_NO_MEMORY, "freezer server is nullptr");
    return freezerServer_->ProxyApp(pidSet, isFreeze);
}

int32_t FreezerServiceStub::ResetAll()
{
    CHECK_AND_RETURN_RET_LOG(freezerServer_ != nullptr, MSERR_NO_MEMORY, "freezer server is nullptr");
    return freezerServer_->ResetAll();
}
