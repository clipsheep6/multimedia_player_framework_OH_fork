...
private:
    bool CheckAudioCapturePermssion();
    bool CheckScreenCapturePermssion();
    int32_t CheckVideoParam(VideoCapInfo videoInfo);
    int32_t CheckAudioParam(AudioCapInfo audioInfo);
    std::shared_ptr<AudioCapturer> CreateAudioCapture(AudioCapInfo audioInfo);
    int32_t StartAudioCap_();
    int32_t StartVideoCap_();
    ...
    VideoCapInfo videoInfo_;

    static constexpr uint32_t MAX_AUDIO_BUFFER_SIZE = 128;
    static constexpr uint64_t SecToNanosecond = 1000000000;
};
} // namespace Media
} // namespace OHOS
