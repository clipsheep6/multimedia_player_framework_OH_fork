...
void TransCoderServer::OnError(ErrorType errorType, int32_t errorCode)
{
...
void TransCoderServer::OnInfo(InfoType type, int32_t extra)
{
    std::lock_guard<std::mutex> lock(cbMutex_);
...
int32_t TransCoderServer::SetInputFile(int32_t fd)
{
    std::lock_guard<std::mutex> lock(mutex_);
    CHECK_STATUS_FAILED_AND_LOGE_RET(status_ != REC_CONFIGURED, MSERR_INVALID_OPERATION);
    CHECK_AND_RETURN_RET_LOG(transCoderEngine_ != nullptr, MSERR_NO_MEMORY, "engine is nullptr");
    config_.srcUrl = fd;
    InputFd inputFileFd(fd);
    auto task = std::make_shared<TaskHandler<int32_t>>([&, this] {
        return transCoderEngine_->Configure(inputFileFd);
    });
    int32_t ret = taskQue_.EnqueueTask(task);
    CHECK_AND_RETURN_RET_LOG(ret == MSERR_OK, ret, "EnqueueTask failed");
...
int32_t TransCoderServer::SetOutputFile(int32_t fd)
{
    ...
    CHECK_AND_RETURN_RET_LOG(transCoderEngine_ != nullptr, MSERR_NO_MEMORY, "engine is nullptr");
    config_.dstUrl = fd;
    OutputFd outFileFd(fd);
    auto task = std::make_shared<TaskHandler<int32_t>>([&, this] {
        return transCoderEngine_->Configure(outFileFd);
    });
    int32_t ret = taskQue_.EnqueueTask(task);
    CHECK_AND_RETURN_RET_LOG(ret == MSERR_OK, ret, "EnqueueTask failed");
