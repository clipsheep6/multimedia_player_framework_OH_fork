...
int32_t AVCodecServiceStub::QueueInputBuffer(uint32_t index, AVCodecBufferInfo info,
                                             AVCodecBufferType type, AVCodecBufferFlag flag)
{
    CHECK_AND_RETURN_RET_LOG(codecServer_ != nullptr, MSERR_NO_MEMORY, "avcodec server is nullptr");
    return codecServer_->QueueInputBuffer(index, info, type, flag);
}
