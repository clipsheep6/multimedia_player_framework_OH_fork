    ...
    MessageOption &option)
{
    switch (code) {
        ...
        case AVCodecListenerMsg::ON_OUTPUT_BUFFER_AVAILABLE: {
            ...
            //OnOutputBufferAvailable(type, extra);
            return MSERR_OK;
        }
        default: {
...
void AVCodecListenerStub::OnError(AVCodecErrorType errorType, int32_t errorCode)
{
    if (callback_ != nullptr) {
        callback_->OnError(static_cast<AVCodecErrorType>(errorType), errorCode);
    }
}
...
void AVCodecListenerStub::OnOutputBufferAvailable(uint32_t index, AVCodecBufferInfo info,
                                                  AVCodecBufferType type, AVCodecBufferFlag flag)
{
    if (callback_ != nullptr) {
        callback_->OnOutputBufferAvailable(index, info, type, flag);
    }
}
