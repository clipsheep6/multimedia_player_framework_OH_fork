...
int32_t AVCodecServer::QueueInputBuffer(uint32_t index, AVCodecBufferInfo info,
                                        AVCodecBufferType type, AVCodecBufferFlag flag)
{
    ...
    CHECK_AND_RETURN_RET_LOG(codecEngine_ != nullptr, MSERR_NO_MEMORY, "engine is nullptr");
    int32_t ret = codecEngine_->QueueInputBuffer(index, info, type, flag);
    if (flag == AVCODEC_BUFFER_FLAG_END_OF_STREAM) {
...
void AVCodecServer::OnOutputBufferAvailable(uint32_t index, AVCodecBufferInfo info,
                                            AVCodecBufferType type, AVCodecBufferFlag flag)
{
    std::lock_guard<std::mutex> lock(cbMutex_);
    if (codecCb_ == nullptr) {
    ...
    codecCb_->OnOutputBufferAvailable(index, info, type, flag);
}
}
}
