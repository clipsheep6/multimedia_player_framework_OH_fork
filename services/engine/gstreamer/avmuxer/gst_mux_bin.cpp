...
static void gst_mux_bin_class_init(GstMuxBinClass *klass)
{
    ...
    g_object_class_install_property(gobject_class, PROP_PARSE,
        g_param_spec_boolean("parse", "Parse", "whether to paese",
    ...
    const GValue *value, GParamSpec *param_spec)
{
    ...
    switch (prop_id) {
        ...
        case PROP_MUX:
            ...
            break;
        case PROP_PARSE:
            mux_bin->parse_ = g_value_get_boolean(value);
            break;
        default:
            break;
    }
    ...
    GValue *value, GParamSpec *param_spec)
{
    ...
    switch (prop_id) {
        ...
        case PROP_MUX:
            ...
            break;
        case PROP_PARSE:
            g_value_set_boolean(value, mux_bin->parse_);
            break;
        default:
            break;
    }
...
static GstStateChangeReturn create_h264parse(GstMuxBin *mux_bin)
{
    ...
    // g_object_set(GST_BASE_PARSE_CAST(mux_bin->h264parse_), "disable-passthrough", false, nullptr);
    // gst_base_parse_set_passthrough(GST_BASE_PARSE_CAST(mux_bin->h264parse_), true);
...
static GstStateChangeReturn gst_mux_bin_change_state(GstElement *element, GstStateChange transition)
{
    ...
    switch (transition) {
        ...
        case GST_STATE_CHANGE_READY_TO_PAUSED:
            if (mux_bin->parse_ == true) {
                if (create_h264parse(mux_bin) != GST_STATE_CHANGE_SUCCESS) {
                    GST_ERROR_OBJECT(mux_bin, "Failed to create parse");
                    return GST_STATE_CHANGE_FAILURE;
                }
                gst_bin_add(GST_BIN(mux_bin), mux_bin->h264parse_);
            }
