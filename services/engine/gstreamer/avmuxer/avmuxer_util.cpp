...
std::map<MimeType, std::tuple<std::string, GType, MyGType>> capsMap = {
    {MUX_H264, {{"alignment", G_TYPE_STRING, "nal"}, {"stream-format", G_TYPE_STRING, "byte-stream"}}},
    {MUX_H263, {}},
    {MUX_MPEG4, {{"mpegversion", G_TYPE_INT, 4}, {"systemstream", G_TYPE_BOOLEAN, FALSE}}},
    {MUX_AAC, {{"mpegversion", G_TYPE_INT, 4}, {"stream-format", G_TYPE_STRING, "adts"}}},
    {MUX_MP3, {{"mpegversion", G_TYPE_INT, 1}, {"layer", G_TYPE_INT, 3}}}
};
...
static void AddCaps(GstCaps *src_caps, MimeType type)
{
    for (auto& elements : capsMap[type]) {
        gst_caps_set_simple(src_caps,
            std::get<0>(elements), std::get<1>(elements), std::get<2>(elements),
            nullptr);
    }
}
