...
static const std::vector<StateTransitionEntry> STATE_TRANSTION_TABLE = {
    ...
    STATE_TRANS_ENTRY(PLAYING, PAUSED, PLAYING, PLAYING),
...
int32_t ChangeStateAction::Execute()
{
    ...
    if (targetGstState_ == GST_STATE_VOID_PENDING) {
    ...
    int32_t ret = stateOperator_.SetGstState(targetGstState_);
...
void ChangeStateAction::HandleMessage(const InnerMessage &inMsg)
{
    ...
    // 最好能记录状态切换的路径，并判断是否这条路径的start为startGstState_, end为targetGstState_
