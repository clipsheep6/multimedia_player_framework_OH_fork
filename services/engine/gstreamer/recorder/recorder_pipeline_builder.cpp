...
int32_t RecorderPipelineBuilder::SetVideoSource(const RecorderSourceDesc &desc)
{
    ...
    if (desc.type_ == VideoSourceType::VIDEO_SOURCE_SURFACE_YUV) {
        videoEncElem_ = CreateElement("VideoEncorder", desc, false);
        ...
        ADD_LINK_DESC(videoSrcElem_, videoEncElem_, "src", "sink", true, true);
...
std::shared_ptr<RecorderPipeline> RecorderPipelineBuilder::Build()
{
    ...
    if (currentCodecFortmat_ == VideoCodecFormat::H264) {
        ...
        for (auto iter = pipelineDesc_->allLinkDescs.begin(); iter != pipelineDesc_->allLinkDescs.end();) {
            if (iter->first->GetName() == "VideoEncorder") {
