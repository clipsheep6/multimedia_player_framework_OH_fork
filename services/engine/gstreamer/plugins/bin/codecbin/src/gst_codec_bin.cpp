...
static gboolean gst_codec_bin_set_profile(GstCodecBin *bin, gint profile);
static gboolean gst_codec_bin_set_bit_mode(GstCodecBin *bin, gint bit_mode);
...
static gboolean operate_element(GstCodecBin *bin)
{
    ...
    if (bin->type == CODEC_BIN_TYPE_VIDEO_ENCODER && !strcmp(bin->coder_name, "openh264enc")) {
        ret = gst_codec_bin_set_profile(bin, bin->codec_profile);
        if (!ret) {
            GST_ERROR_OBJECT(bin, "Failed to set_profile");
            return ret;
        }
        g_object_set(bin->coder, "profile", bin->codec_profile, nullptr);
        ret = gst_codec_bin_set_bit_mode(bin, bin->bitrate_mode);
        if (!ret) {
            GST_ERROR_OBJECT(bin, "Failed to bit_mode");
            return ret;
        }
        ...
        if (G_IS_PARAM_SPEC_ENUM(param)) {
            ...
            g_object_set(bin->coder, "rate-control", values[bin->bitrate_mode].value, nullptr);
        }
    }
...
static gboolean gst_codec_bin_set_profile(GstCodecBin *bin, gint profile)
{
    ...
    switch (profile) {
        ...
        case CODEC_BIN_H264_PROFILE_MAIN:
            ...
            break;
        default:
            GST_ERROR_OBJECT(bin, "set profile(%d) fail", profile);
            break;
    }
...
static gboolean gst_codec_bin_set_bit_mode(GstCodecBin *bin, gint bit_mode)
{
    ...
    switch (bit_mode) {
        ...
        case CODEC_BIN_H264_BIT_MODE_CQ:
            ...
            break;
        default:
            GST_ERROR_OBJECT(bin, "set bit_mode(%d) fail", bit_mode);
            break;
    }
