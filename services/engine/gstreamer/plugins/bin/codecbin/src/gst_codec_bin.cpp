    ...
    const GValue *value)
{
    ...
    switch (prop_id) {
        ...
        case PROP_USE_SURFACE_OUTPUT:
            ...
            break;
        case PROP_DFX_NODE:
            bin->dfx_node = *(reinterpret_cast<std::shared_ptr<OHOS::Media::DfxNode> *>(g_value_get_boolean(value)));
            break;
        default:
            break;
    }
    ...
    if (bin->dfx_node != nullptr) {
        if (bin->type == CODEC_BIN_TYPE_VIDEO_DECODER && bin->use_software == FALSE) {
            std::shared_ptr<OHOS::Media::DfxNode> node =
                OHOS::Media::DfxNodeManager::GetInstance().CreateChildDfxNode(bin->dfx_node, "DECODER");
            g_object_set(bin->coder, "dfx-node", &node, nullptr);        
        }
        if (bin->type == CODEC_BIN_TYPE_VIDEO_ENCODER && bin->use_software == FALSE) {
            std::shared_ptr<OHOS::Media::DfxNode> node =
                OHOS::Media::DfxNodeManager::GetInstance().CreateChildDfxNode(bin->dfx_node, "ENCODER");
            g_object_set(bin->coder, "dfx-node", &node, nullptr);        
        }
