    ...
    int32_t offset, int32_t length, FreeMemory free_memory)
{
    ...
    GstShMemoryWrapMemory *memory = reinterpret_cast<GstShMemoryWrapMemory *>(g_slice_alloc0(sizeof(GstShMemoryWrapMemory)));
...
{
    g_return_val_if_fail(mem != nullptr && reinterpret_cast<GstShMemoryWrapMemory *>(mem)->shmemory != nullptr, nullptr);
...
static GstMemory *gst_shmemory_wrap_allocator_mem_copy(GstShMemoryWrapMemory *mem, gssize offset, gssize size)
{
    ...
    GST_DEBUG("in gst_shmemory_wrap_allocator_mem_copy, offset is %" G_GSSIZE_FORMAT ", size is %" G_GSSIZE_FORMAT "", offset, size);
