...
static void gst_video_capture_src_init(GstVideoCaptureSrc *videocapturesrc)
{
    ...
    gst_base_src_set_format(GST_BASE_SRC(videocapturesrc), GST_FORMAT_TIME);
    gst_base_src_set_live(GST_BASE_SRC(videocapturesrc), TRUE);

    // private param
    videocapturesrc->video_frame_rate = DEFAULT_FRAME_RATE;
    videocapturesrc->is_start = false;
...
static GstStateChangeReturn gst_video_capture_src_change_state(GstElement *element, GstStateChange transition)
{
    ...
    switch (transition) {
        case GST_STATE_CHANGE_NULL_TO_READY:
            // 使用父类的state change 来创建surface 和 init pool
            break;
        case GST_STATE_CHANGE_READY_TO_PAUSED:
            break;
        case GST_STATE_CHANGE_PAUSED_TO_PLAYING:
            if (capturesrc->is_start == false) {
                // g_return_val_if_fail(start_video_capture(src) == TRUE, GST_STATE_CHANGE_FAILURE);
                capturesrc->is_start = true;
            } else {
...
static GstFlowReturn gst_video_capture_src_fill(GstBaseSrc *src, guint64 offset, guint size, GstBuffer *buf)
{
    ...
    GST_INFO("video capture buffer size is: %d, pts: % " G_GINT64_FORMAT "", meta->length, timestamp);
    GST_INFO("is this buffer keyFrame ? : %d", is_codec_buffer);
    // timestamp - pausetime
    // GST_BUFFER_PTS (buf) = newtimestamp;
