...
int32_t HdiCodec::Init()
{
    ...
    auto ret = HdiInit::GetInstance().GetHandle(&handle_, componentName_, appData_, callback_);
...
void HdiCodec::Deinit()
{
    ...
    auto ret = HdiInit::GetInstance().FreeHandle(handle_);
    handle_ = nullptr;
    if (ret != HDF_SUCCESS) {
...
int32_t HdiCodec::EmptyBufferDone(CodecCallbackType *self, int8_t *appData,
        uint32_t appDataLen, const OmxCodecBuffer *buffer)
{
    MEDIA_LOGD("EmptyBufferDone");
    CHECK_AND_RETURN_RET_LOG(appData != nullptr, HDF_ERR_INVALID_PARAM, "appData is null");
...
int32_t HdiCodec::FillBufferDone(CodecCallbackType *self, int8_t *appData,
        uint32_t appDataLen, OmxCodecBuffer *buffer)
{
    MEDIA_LOGD("FillBufferDone");
    CHECK_AND_RETURN_RET_LOG(appData != nullptr, OMX_ErrorBadParameter, "appData is null");
    AppData *appdata = reinterpret_cast<AppData *>(appData);
    auto instance = appdata->instance.lock();
