...
int32_t HdiInit::GetHandle(CodecComponentType **component, std::string name,
    OMX_PTR appData, CodecCallbackType *callbacks)
{
    ...
    return mgr_->CreateComponent(component, const_cast<char *>(name.c_str()), appData, sizeof(appData), callbacks);
}

int32_t HdiInit::FreeHandle(CodecComponentType *component)
{
    CHECK_AND_RETURN_RET_LOG(mgr_ != nullptr, HDF_FAILURE, "mgr_ is nullptr");
    CHECK_AND_RETURN_RET_LOG(component != nullptr, HDF_FAILURE, "component is nullptr");
    return mgr_->DestoryComponent(component);
}
}  // namespace Media
}  // namespace OHOS
