...
inline int32_t HdiSetParameter(T *handle, enum OMX_INDEXTYPE paramIndex, U &param)
{
    return handle->SetParameter(handle, paramIndex, (int8_t *)&param, sizeof(param));
}

template <typename T, typename U>
inline int32_t HdiGetParameter(T *handle, enum OMX_INDEXTYPE paramIndex, U &param)
{
    return handle->GetParameter(handle, paramIndex, (int8_t *)&param, sizeof(param));
}

template <typename T, typename U>
inline int32_t HdiGetConfig(T *handle, enum OMX_INDEXTYPE paramIndex, U &param)
{
    return handle->GetConfig(handle, paramIndex, (int8_t *)&param, sizeof(param));
}

template <typename T, typename U>
inline int32_t HdiSetConfig(T *handle, enum OMX_INDEXTYPE paramIndex, U &param)
{
