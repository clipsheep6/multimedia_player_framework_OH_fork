...
namespace {
    ...
    constexpr gint64 DEFAULT_AUDIO_JITTER_THD = 20000000; // ns, 20ms
...
static GstClockTime gst_mem_sink_adjust_reach_time_by_jitter(GstMemSink *mem_sink, GstClockTime reach_time)
{
    ...
    g_mutex_lock(&priv->mutex);
    if (priv->audio_sink != nullptr) {
        gint64 audio_jitter = 0;
        g_object_get(priv->audio_sink, "current-jitter", &audio_jitter, nullptr);
        if (audio_jitter > DEFAULT_AUDIO_JITTER_THD) {
            GST_LOG_OBJECT(mem_sink, "audio jitter=%" G_GINT64_FORMAT
                ", old reach_time=%" G_GUINT64_FORMAT ", new reach_time=%" G_GUINT64_FORMAT,
                audio_jitter, reach_time, reach_time + audio_jitter);
            reach_time += audio_jitter;
        }
    }
    g_mutex_unlock(&priv->mutex);
    return reach_time;
...
static GstClockTime gst_mem_sink_update_reach_time(GstBaseSink *base_sink, GstClockTime reach_time)
{
    ...
    // 2ed: update reach_time if the running_time_diff over limmit
    ...
    if (running_time_diff > DEFAULT_MAX_WAIT_CLOCK_TIME) {
        new_reach_time = new_reach_time - (running_time_diff - DEFAULT_MAX_WAIT_CLOCK_TIME);
    }
    if (new_reach_time != reach_time) {
        GST_INFO_OBJECT(base_sink, "running_time_diff:%" G_GUINT64_FORMAT " old reach_time:%" G_GUINT64_FORMAT
