...
gboolean gst_surface_pool_set_surface(GstSurfacePool *pool, OHOS::sptr<OHOS::Surface> surface, guint waittime)
{
    ...
    if (pool->started) {
        GST_ERROR("started already, reject to set surface");
        GST_OBJECT_UNLOCK(pool);
        return FALSE;
    }

    if (pool->surface != nullptr) {
        GST_ERROR("surface has already been set");
        GST_OBJECT_UNLOCK(pool);
        return FALSE;
    }
    ...
    GstBufferPoolAcquireParams *params, GstSurfaceMemory **memory)
{
    ...
    *memory = nullptr;
    ...
    /**
     * Rather than setting the timeout parameter of Surface's RequestBuffer, We set up
     * a tick to constantly check whether there is a SurfaceBuffer available on the
     * Surface for avoiding dependence on the timeout mechanism of the Surface
     */
    while (TRUE) {
    ...
    GstBuffer **buffer, GstBufferPoolAcquireParams *params)
{
    ...
    *buffer = gst_buffer_new();
    if (*buffer == nullptr) {
        ...
        gst_surface_allocator_free(spool->allocator, memory);
