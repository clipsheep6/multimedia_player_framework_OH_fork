...
namespace {
    ...
    constexpr uint64_t DEFAULT_EXTRA_AUDIO_DELAY_US = 220000;
}
...
static GstFlowReturn gst_audio_server_sink_render(GstBaseSink *basesink, GstBuffer *buffer)
{
    ...
    if (sink->frame_after_segment) {
        ...
        if (sink->audio_sink->GetLatency(latency) != MSERR_OK) {
            ...
            GST_INFO_OBJECT(basesink, "frame render latency is (%" PRIu64 "), add (%" PRIu64 ")", latency, DEFAULT_EXTRA_AUDIO_DELAY_US);
            latency += DEFAULT_EXTRA_AUDIO_DELAY_US;
