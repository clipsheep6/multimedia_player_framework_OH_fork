...
namespace {
    ...
    constexpr uint64_t DEFAULT_AUDIO_RENDER_DELAY = 325000000; // unit ns, empirical value
}
...
static void gst_audio_server_sink_init(GstAudioServerSink *sink)
{
    ...
    sink->delay_time = DEFAULT_AUDIO_RENDER_DELAY;
}
    ...
    switch (prop_id) {
        ...
        case PROP_DELAY_TIME:
            ...
            if (sink->enable_opt_render_delay) {
                g_value_set_uint(value, sink->delay_time);
            } else {
