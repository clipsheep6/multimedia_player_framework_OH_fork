...
uint64_t GstPlayerCtrl::GetPosition()
{
    ...
    MEDIA_LOGD("GetPosition curTime(0x%{public}" PRIu64 ") duration(0x%{public}" PRIu64 ")", curTime, sourceDuration_);
    return curTime;
}
...
void GstPlayerCtrl::ProcessSeekDone(const GstPlayer *cbPlayer, uint64_t position)
{
    ...
    MEDIA_LOGI("gstplay seek Done: (0x%{public}" PRIu64 ")", seekDonePosition_);
}
...
void GstPlayerCtrl::ProcessPositionUpdated(const GstPlayer *cbPlayer, uint64_t position)
{
    ...
    if (tempObs != nullptr) {
        MEDIA_LOGD("ProcessPositionUpdated(0x%{public}" PRIu64 "), 0x%{public}06" PRIXPTR "", position, FAKE_POINTER(this));
...
{
    if (seekDoneNeedCb_) {
        MEDIA_LOGI("On Seek Done: (0x%{public}" PRIu64 ")", seekDonePosition_);
        std::shared_ptr<IPlayerEngineObs> tempObs = obs_.lock();
        Format format;
        if (tempObs != nullptr) {
...
void GstPlayerCtrl::InitDuration()
{
    ...
    MEDIA_LOGD("InitDuration duration(0x%{public}" PRIu64 ")", sourceDuration_);
}
} // Media
} // OHOS