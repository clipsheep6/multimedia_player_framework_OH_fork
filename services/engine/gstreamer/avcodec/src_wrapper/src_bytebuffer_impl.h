...
public:
    virtual ~SrcBytebufferImpl() = default;

    std::shared_ptr<AVSharedMemory> GetInputBuffer(uint32_t index) override;
    int32_t QueueInputBuffer(uint32_t index, AVCodecBufferInfo info,
                             AVCodecBufferType type, AVCodecBufferFlag flag) override;
    int32_t SetParameter(const Format &format) override;
    int32_t SetCallback(const std::shared_ptr<SrcCallback> &callback) override;
