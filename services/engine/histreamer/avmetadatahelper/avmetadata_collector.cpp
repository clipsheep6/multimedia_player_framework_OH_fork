...
static constexpr int artPictureMaxSize = 1024 * 1024;
static constexpr int secondDividMs = 1000;
}  // namespace

namespace OHOS {
namespace Media {
static const std::unordered_map<Plugins::FileType, std::string> fileTypeMap = {{Plugins::FileType::UNKNOW, "uknown"},
    ...
    const std::shared_ptr<Meta> &globalInfo, const std::vector<std::shared_ptr<Meta>> &trackInfos)
{
    ...
    while (it != metadata.tbl_.end()) {
        MEDIA_LOGD("metadata tbl, key: %{public}d, keyName: %{public}s, val: %{public}s",
            it->first,
            AVMETA_KEY_TO_X_MAP.find(it->first)->second.c_str(),
            it->second.c_str());
        it++;
    }
    return metadata.tbl_;
