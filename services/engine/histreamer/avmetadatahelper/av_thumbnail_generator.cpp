...
constexpr int32_t RATE_UV_DEVIDE = 3;
...
{
    MediaTrace trace("M:InitDecoder");
...
void AVThumbnailGenerator::OnOutputBufferAvailable(uint32_t index, std::shared_ptr<AVBuffer> buffer)
{
    ...
    if (!hasFetchedFrame_.load() && !stopProcessing_.load()) {
        MediaTrace::TraceEnd("M:WaitBuffer", FAKE_POINTER(this));
        bufferIndex_ = index;
                                                                       ...
                                                                       const OutputConfiguration &param)
{
    ...
    MediaTrace::TraceBegin("M:WaitBuffer", FAKE_POINTER(this));
    {
...
{
    MediaTrace trace("M:ConvertToAVSharedMemory");
    ...
    fetchedFrameAtTime_ = std::make_shared<AVSharedMemoryBase>(
        sizeof(OutputFrame) + surfaceBuffer->GetWidth() * surfaceBuffer->GetHeight() * RATE_UV_DEVIDE / RATE_UV,
    ...
    frame->stride_ = surfaceBuffer->GetStride();
    frame->bytesPerPixel_ = RATE_UV;
    frame->size_ = surfaceBuffer->GetWidth() * surfaceBuffer->GetHeight() * RATE_UV;
    frame->rotation_ = static_cast<int32_t>(rotation_);
    GetYuvDataAlignStride(surfaceBuffer);
    return true;
}
...
void AVThumbnailGenerator::GetYuvDataAlignStride(const sptr<SurfaceBuffer> &surfaceBuffer)
{
    ...
    uint8_t *dstPtr = sizeof(OutputFrame) + fetchedFrameAtTime_->GetBase();
    int32_t format = surfaceBuffer->GetFormat();
