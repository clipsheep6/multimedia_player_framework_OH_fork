...
namespace OHOS {
namespace Media {
AudioEncoderCallbackNapi::AudioEncoderCallbackNapi(napi_env env, std::shared_ptr<AudioEncoder> audioEncoderImpl)
    : env_(env),
      audioEncoderImpl_(audioEncoderImpl)
{
...
void AudioEncoderCallbackNapi::SaveCallbackReference(const std::string &callbackName, napi_value args)
{
...
std::shared_ptr<AutoRef> AudioEncoderCallbackNapi::StateCallbackSelect(const std::string &callbackName) const
{
...
void AudioEncoderCallbackNapi::SendStateCallback(const std::string &callbackName)
{
...
void AudioEncoderCallbackNapi::OnInputBufferAvailable(uint32_t index)
{
    ...
    CHECK_AND_RETURN_LOG(audioEncoderImpl_ != nullptr, "Failed to retrieve instance");
    auto buffer = audioEncoderImpl_->GetInputBuffer(index);
...
void AudioEncoderCallbackNapi::OnOutputBufferAvailable(uint32_t index, AVCodecBufferInfo info, AVCodecBufferFlag flag)
{
    ...
    CHECK_AND_RETURN_LOG(audioEncoderImpl_ != nullptr, "Failed to retrieve instance");
    auto buffer = audioEncoderImpl_->GetOutputBuffer(index);
...
void AudioEncoderCallbackNapi::OnJsStateCallBack(AudioEncoderJsCallback *jsCb) const
{
...
void AudioEncoderCallbackNapi::OnJsInputCallBack(AudioEncoderJsCallback *jsCb) const
{
    uv_loop_s *loop = nullptr;
    napi_get_uv_event_loop(env_, &loop);
    if (loop == nullptr) {
...
void AudioEncoderCallbackNapi::OnJsOutputCallBack(AudioEncoderJsCallback *jsCb) const
{
    uv_loop_s *loop = nullptr;
    napi_get_uv_event_loop(env_, &loop);
    if (loop == nullptr) {
