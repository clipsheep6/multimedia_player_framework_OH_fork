...
napi_value AudioEncoderNapi::Init(napi_env env, napi_value exports)
{
    ...
    napi_property_descriptor staticProperty[] = {
        DECLARE_NAPI_STATIC_FUNCTION("createAudioEncoder", CreateAudioEncoder),
    };
...
napi_value AudioEncoderNapi::Constructor(napi_env env, napi_callback_info info)
{
    ...
    CHECK_AND_RETURN_RET_LOG(audioEncoderNapi != nullptr, nullptr, "No memory");

    audioEncoderNapi->env_ = env;
    audioEncoderNapi->audioEncoderImpl_ = AudioEncoderFactory::CreateByCodecName("todo");
    CHECK_AND_RETURN_RET_LOG(audioEncoderNapi->audioEncoderImpl_ != nullptr, nullptr, "No memory");

    audioEncoderNapi->taskQue_ = std::make_unique<TaskQueue>("AudioEncoderNapi");
    (void)audioEncoderNapi->taskQue_->Start();

    audioEncoderNapi->callbackNapi_ = std::make_shared<AudioEncoderCallbackNapi>(env);
...
napi_value AudioEncoderNapi::CreateAudioEncoder(napi_env env, napi_callback_info info)
{
    napi_value result = nullptr;
    napi_value constructor = nullptr;
    ...
    MEDIA_LOGD("CreateAudioEncoder success");
    return result;
}
