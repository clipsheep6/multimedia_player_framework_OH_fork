...
void AudioDecoderCallbackNapi::OnStreamChanged(const Format &format)
{
    std::lock_guard<std::mutex> lock(mutex_);
...
void AudioDecoderCallbackNapi::OnNeedInputData(uint32_t index)
{
    std::lock_guard<std::mutex> lock(mutex_);
...
void AudioDecoderCallbackNapi::OnNewOutputData(uint32_t index, AVCodecBufferInfo info, AVCodecBufferFlag flag)
{
    std::lock_guard<std::mutex> lock(mutex_);
