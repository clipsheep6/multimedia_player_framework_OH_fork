...
namespace OHOS {
namespace Media {
AudioDecoderCallbackNapi::AudioDecoderCallbackNapi(napi_env env, std::shared_ptr<AudioDecoder> audioDecoderImpl)
    : env_(env),
      audioDecoderImpl_(audioDecoderImpl)
{
...
std::shared_ptr<AutoRef> AudioDecoderCallbackNapi::StateCallbackSelect(const std::string &callbackName) const
{
...
void AudioDecoderCallbackNapi::SendStateCallback(const std::string &callbackName)
{
...
void AudioDecoderCallbackNapi::OnInputBufferAvailable(uint32_t index)
{
    ...
    CHECK_AND_RETURN_LOG(audioDecoderImpl_ != nullptr, "Failed to retrieve instance");
    auto buffer = audioDecoderImpl_->GetInputBuffer(index);
...
void AudioDecoderCallbackNapi::OnOutputBufferAvailable(uint32_t index, AVCodecBufferInfo info, AVCodecBufferFlag flag)
{
    ...
    CHECK_AND_RETURN_LOG(audioDecoderImpl_ != nullptr, "Failed to retrieve instance");
    auto buffer = audioDecoderImpl_->GetOutputBuffer(index);
...
void AudioDecoderCallbackNapi::OnJsStateCallBack(AudioDecoderJsCallback *jsCb) const
{
...
void AudioDecoderCallbackNapi::OnJsInputCallBack(AudioDecoderJsCallback *jsCb) const
{
    uv_loop_s *loop = nullptr;
    napi_get_uv_event_loop(env_, &loop);
    if (loop == nullptr) {
...
void AudioDecoderCallbackNapi::OnJsOutputCallBack(AudioDecoderJsCallback *jsCb) const
{
    uv_loop_s *loop = nullptr;
    napi_get_uv_event_loop(env_, &loop);
    if (loop == nullptr) {
