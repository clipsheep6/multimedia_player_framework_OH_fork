...
napi_value AudioDecoderNapi::Init(napi_env env, napi_value exports)
{
    ...
    napi_property_descriptor staticProperty[] = {
        ...
        DECLARE_NAPI_STATIC_FUNCTION("createAudioDecoderByCodec", CreateAudioDecoderByCodec),
    };
...
napi_value AudioDecoderNapi::Constructor(napi_env env, napi_callback_info info)
{
    ...
    CHECK_AND_RETURN_RET_LOG(audioDecoderNapi != nullptr, undefined, "No memory");
    ...
    switch (static_cast<AudioDecoderCreator>(creator)) {
        case AUDIO_DECODER_CREATOR_BY_NAME:
            audioDecoderNapi->audioDecoderImpl_ = AudioDecoderFactory::CreateByDecoderName(str);
            break;
        case AUDIO_DECODER_CREATOR_BY_CODEC:
            audioDecoderNapi->audioDecoderImpl_ = AudioDecoderFactory::CreateByMimeType(str);
            break;
        default:
            break;
    }

    if (audioDecoderNapi->audioDecoderImpl_ == nullptr) {
    ...
    audioDecoderNapi->callbackNapi_ = std::make_shared<AudioDecoderCallbackNapi>(env, audioDecoderNapi->audioDecoderImpl_);
...
napi_value AudioDecoderNapi::CreateAudioDecoderByName(napi_env env, napi_callback_info info)
{
    ...
    status = napi_create_int32(env, static_cast<int32_t>(AUDIO_DECODER_CREATOR_BY_NAME), &args[1]);
    ...
    MEDIA_LOGD("CreateAudioDecoderByName success");
    return result;
}

napi_value AudioDecoderNapi::CreateAudioDecoderByCodec(napi_env env, napi_callback_info info)
{
    napi_value result = nullptr;
    napi_value constructor = nullptr;
    napi_status status = napi_get_reference_value(env, constructor_, &constructor);
    ...
    status = napi_create_int32(env, static_cast<int32_t>(AUDIO_DECODER_CREATOR_BY_CODEC), &args[1]);
    if (status != napi_ok || args[1] == nullptr) {
        MEDIA_LOGE("Failed to create int32");
        napi_get_undefined(env, &result);
        return result;
    ...
    MEDIA_LOGD("CreateAudioDecoderByCodec success");
    return result;
}
...
napi_value AudioDecoderNapi::Stop(napi_env env, napi_callback_info info)
{
    ...
    CHECK_AND_RETURN_RET_LOG(audioDecoderNapi->audioDecoderImpl_ != nullptr, undefinedResult, "No Impl");
    CHECK_AND_RETURN_RET_LOG(audioDecoderNapi->taskQue_ != nullptr, undefinedResult, "No TaskQue");
    auto task = std::make_shared<TaskHandler<void>>([napi = audioDecoderNapi]() {
...
napi_value AudioDecoderNapi::QueueInputBuffer(napi_env env, napi_callback_info info)
{
    napi_value undefinedResult = nullptr;
    napi_get_undefined(env, &undefinedResult);
    ...
    std::unique_ptr<AudioDecoderAsyncContext> asyncContext = std::make_unique<AudioDecoderAsyncContext>();
    CHECK_AND_RETURN_RET_LOG(asyncContext != nullptr, undefinedResult, "No memory");
    status = napi_unwrap(env, jsThis, reinterpret_cast<void **>(&asyncContext->audioDecoderNapi));
    CHECK_AND_RETURN_RET_LOG(status == napi_ok && asyncContext->audioDecoderNapi != nullptr, undefinedResult,
        "Failed to retrieve instance");
    ...
    status = napi_create_async_work(
        ...
        [](napi_env env, void *data) {
            auto context = static_cast<AudioDecoderAsyncContext *>(data);
            context->status = context->audioDecoderNapi->audioDecoderImpl_->
...
napi_value AudioDecoderNapi::ReleaseOutputBuffer(napi_env env, napi_callback_info info)
{
    napi_value undefinedResult = nullptr;
    napi_get_undefined(env, &undefinedResult);
...
void AudioDecoderNapi::CommonCallbackRoutine(napi_env env, AudioDecoderAsyncContext* &asyncContext,
...
void AudioDecoderNapi::SetFunctionAsyncCallbackComplete(napi_env env, napi_status status, void *data)
{
    auto asyncContext = static_cast<AudioDecoderAsyncContext*>(data);
    ...
    if (asyncContext != nullptr) {
        ...
        MEDIA_LOGE("AudioDecoderAsyncContext* is nulltr");
    }
}
