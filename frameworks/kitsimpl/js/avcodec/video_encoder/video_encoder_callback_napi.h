...
public:
    explicit VideoEncoderCallbackNapi(napi_env env, std::shared_ptr<VideoEncoder> audioEncoderImpl);
...
private:
    ...
    void OnJsStateCallBack(VideoEncoderJsCallback *jsCb) const;
    void OnJsInputCallBack(VideoEncoderJsCallback *jsCb) const;
    void OnJsOutputCallBack(VideoEncoderJsCallback *jsCb) const;
    std::shared_ptr<AutoRef> StateCallbackSelect(const std::string &callbackName) const;
    napi_env env_ = nullptr;
    std::shared_ptr<VideoEncoder> videoEncoderImpl_ = nullptr;
    std::mutex mutex_;
