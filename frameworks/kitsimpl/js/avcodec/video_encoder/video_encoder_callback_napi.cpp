...
namespace OHOS {
namespace Media {
VideoEncoderCallbackNapi::VideoEncoderCallbackNapi(napi_env env, std::shared_ptr<VideoEncoder> videoEncoderImpl)
    : env_(env),
      videoEncoderImpl_(videoEncoderImpl)
{
...
std::shared_ptr<AutoRef> VideoEncoderCallbackNapi::StateCallbackSelect(const std::string &callbackName) const
{
...
void VideoEncoderCallbackNapi::SendStateCallback(const std::string &callbackName)
{
...
void VideoEncoderCallbackNapi::OnInputBufferAvailable(uint32_t index)
{
    ...
    CHECK_AND_RETURN_LOG(videoEncoderImpl_ != nullptr, "Failed to retrieve instance");
    auto buffer = videoEncoderImpl_->GetInputBuffer(index);
...
void VideoEncoderCallbackNapi::OnOutputBufferAvailable(uint32_t index, AVCodecBufferInfo info, AVCodecBufferFlag flag)
{
    ...
    CHECK_AND_RETURN_LOG(videoEncoderImpl_ != nullptr, "Failed to retrieve instance");
    auto buffer = videoEncoderImpl_->GetOutputBuffer(index);
...
void VideoEncoderCallbackNapi::OnJsStateCallBack(VideoEncoderJsCallback *jsCb) const
{
...
void VideoEncoderCallbackNapi::OnJsInputCallBack(VideoEncoderJsCallback *jsCb) const
{
    uv_loop_s *loop = nullptr;
    napi_get_uv_event_loop(env_, &loop);
    if (loop == nullptr) {
...
void VideoEncoderCallbackNapi::OnJsOutputCallBack(VideoEncoderJsCallback *jsCb) const
{
    uv_loop_s *loop = nullptr;
    napi_get_uv_event_loop(env_, &loop);
    if (loop == nullptr) {
