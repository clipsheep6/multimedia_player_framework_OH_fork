...
napi_value VideoEncoderNapi::Init(napi_env env, napi_value exports)
{
    ...
    napi_property_descriptor staticProperty[] = {
        ...
        DECLARE_NAPI_STATIC_FUNCTION("createVideoEncoderByCodec", CreateVideoEncoderByCodec),
    };
...
napi_value VideoEncoderNapi::Constructor(napi_env env, napi_callback_info info)
{
    ...
    CHECK_AND_RETURN_RET_LOG(videoEncoderNapi != nullptr, undefined, "No memory");
    ...
    switch (static_cast<VideoEncoderCreator>(creator)) {
        case VIDEO_ENCODER_CREATOR_BY_NAME:
            videoEncoderNapi->videoEncoderImpl_ = VideoEncoderFactory::CreateByEncoderName(str);
            break;
        case VIDEO_ENCODER_CREATOR_BY_CODEC:
            videoEncoderNapi->videoEncoderImpl_ = VideoEncoderFactory::CreateByMimeType(str);
            break;
        default:
            break;
    }

    if (videoEncoderNapi->videoEncoderImpl_ == nullptr) {
    ...
    videoEncoderNapi->callbackNapi_ = std::make_shared<VideoEncoderCallbackNapi>(env, videoEncoderNapi->videoEncoderImpl_);
...
napi_value VideoEncoderNapi::CreateVideoEncoderByName(napi_env env, napi_callback_info info)
{
    ...
    status = napi_create_int32(env, static_cast<int32_t>(VIDEO_ENCODER_CREATOR_BY_NAME), &args[1]);
    ...
    MEDIA_LOGD("CreateVideoEncoderByName success");
    return result;
}

napi_value VideoEncoderNapi::CreateVideoEncoderByCodec(napi_env env, napi_callback_info info)
{
    napi_value result = nullptr;
    napi_value constructor = nullptr;
    napi_status status = napi_get_reference_value(env, constructor_, &constructor);
    ...
    status = napi_create_int32(env, static_cast<int32_t>(VIDEO_ENCODER_CREATOR_BY_CODEC), &args[1]);
    if (status != napi_ok || args[1] == nullptr) {
        MEDIA_LOGE("Failed to create int32");
        napi_get_undefined(env, &result);
        return result;
    ...
    MEDIA_LOGD("CreateVideoEncoderByCodec success");
    return result;
}
...
napi_value VideoEncoderNapi::QueueInputBuffer(napi_env env, napi_callback_info info)
{
    napi_value undefinedResult = nullptr;
    napi_get_undefined(env, &undefinedResult);
    ...
    std::unique_ptr<VideoEncoderAsyncContext> asyncContext = std::make_unique<VideoEncoderAsyncContext>();
    CHECK_AND_RETURN_RET_LOG(asyncContext != nullptr, undefinedResult, "No memory");
    status = napi_unwrap(env, jsThis, reinterpret_cast<void **>(&asyncContext->videoEncoderNapi));
    CHECK_AND_RETURN_RET_LOG(status == napi_ok && asyncContext->videoEncoderNapi != nullptr, undefinedResult,
        "Failed to retrieve instance");
    ...
    status = napi_create_async_work(
        ...
        [](napi_env env, void *data) {
            auto context = static_cast<VideoEncoderAsyncContext *>(data);
            context->status = context->videoEncoderNapi->videoEncoderImpl_->
...
napi_value VideoEncoderNapi::ReleaseOutputBuffer(napi_env env, napi_callback_info info)
{
    napi_value undefinedResult = nullptr;
    napi_get_undefined(env, &undefinedResult);
...
void VideoEncoderNapi::CommonCallbackRoutine(napi_env env, VideoEncoderAsyncContext* &asyncContext,
...
void VideoEncoderNapi::SetFunctionAsyncCallbackComplete(napi_env env, napi_status status, void *data)
{
    auto asyncContext = static_cast<VideoEncoderAsyncContext*>(data);
    ...
    if (asyncContext != nullptr) {
        ...
        MEDIA_LOGE("VideoEncoderAsyncContext* is nulltr");
    }
}
