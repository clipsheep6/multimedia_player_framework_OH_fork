...
napi_value CommonNapi::CreateFormatBuffer(napi_env env, Format &format)
{
    ...
    for (auto &iter :  FORMAT_DATA) {
        if (iter.second == FORMAT_TYPE_INT32) {
            ...
            if (ret == true) {
                CHECK_AND_RETURN_RET(AddNumberProperty(env, buffer, iter.first, value) == true, nullptr);
            }
        } else if (iter.second == FORMAT_TYPE_STRING) {
            std::string value;
            bool ret = format.GetStringValue(iter.first, value);
            if (ret == true) {
                CHECK_AND_RETURN_RET(AddStringProperty(env, buffer, iter.first, value) == true, nullptr);
            }
...
{
    MEDIA_LOGD("MediaJsResultArray array size:%{public}d", value_.size());
    // create Description
