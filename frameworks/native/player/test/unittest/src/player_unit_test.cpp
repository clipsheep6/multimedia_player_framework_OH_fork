...
void PlayerUnitTest::PlayFunTest(const std::string &protocol)
{
    ...
    if (player_ != nullptr) {
        ...
        if (protocol == PlayerTestParam::HLS_PLAY) {
            EXPECT_EQ(MSERR_OK, player_->SelectBitRate(200000));
            sleep(PLAYING_TIME);
        }
        sleep(PLAYING_TIME);
...
HWTEST_F(PlayerUnitTest, Player_Http_001, TestSize.Level2)
{
    ...
    EXPECT_EQ(MSERR_OK, ret);if (ret == MSERR_OK) {
...
{
    int32_t ret = player_->SetSource("http://devimages.apple.com/iphone/samples/bipbop/bipbopall.m3u8");
    EXPECT_EQ(MSERR_OK, ret);
    ret = player_->PrepareAsync();
    EXPECT_EQ(MSERR_OK, ret);
    if (ret == MSERR_OK) {
 ...
 * @tc.desc  : Test Player SetPlaybackSpeed
 */
HWTEST_F(PlayerUnitTest, Player_SetPlaybackSpeed_003, TestSize.Level2)
{
    int32_t duration = 0;
    PlaybackRateMode mode;
    ASSERT_EQ(MSERR_OK, player_->SetSource(VIDEO_FILE1));
    ...
    EXPECT_EQ(MSERR_OK, player_->Play());
    sleep(PLAYING_TIME);
    EXPECT_EQ(MSERR_OK, player_->SetPlaybackSpeed(SPEED_FORWARD_1_00_X));
    EXPECT_EQ(MSERR_OK, player_->GetPlaybackSpeed(mode));
    EXPECT_EQ(MSERR_OK, player_->SetPlaybackSpeed(SPEED_FORWARD_1_00_X));
    EXPECT_EQ(MSERR_OK, player_->GetPlaybackSpeed(mode));
