...
{
    MEDIA_LOGI("PreLoad hapticUri [%{public}s], streamUsage [%{public}d]",
        hapticSource.hapticUri_.c_str(), streamUsage);
    ...
    if (hapticSource.hapticUri_ == "") {
        bool state = false;
        if (Sensors::IsSupportEffect(hapticSource.effectId_.c_str(), &state) == 0) {
            SetIsSupportEffectId(true);
            MEDIA_LOGW("only for effectId source.");
            return MSERR_OK;
        } else {
    ...
    int32_t fd = open(hapticSource.hapticUri_.c_str(), O_RDONLY);
    if (fd == -1) {
...
{
    MEDIA_LOGI("SetHapticIntensity for clock %{public}f", intensity);
...
int32_t AudioHapticVibratorImpl::StartVibrate(const AudioLatencyMode &latencyMode)
{
    ...
    if (latencyMode == AUDIO_LATENCY_MODE_NORMAL) {
        ...
        if (IsSupportEffectId_) {
            ...
            result = Sensors::PlayPrimitiveEffect(hapticSource_effectId.c_str(), vibrateIntensity_);
            MEDIA_LOGD("StartVibrate effectId: %{public}s, result: %{public}d", hapticSource_effectId.c_str(), result);
        } else {
