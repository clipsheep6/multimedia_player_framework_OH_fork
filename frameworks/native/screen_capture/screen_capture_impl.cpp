...
bool ScreenCaptureImpl::NeedStartInnerAudio(AudioCapSourceType type) {
    CHECK_AND_RETURN_RET_LOG(type != PLAYBACK, true, "Need start Inner AudioCapture");
    return false;
}

int32_t ScreenCaptureImpl::SetMicrophoneEnable(bool isMicrophone)
{
    MEDIA_LOGD("SetMicrophoneEnable:0x%{public}06" PRIXPTR " init in", FAKE_POINTER(this));
    CHECK_AND_RETURN_RET_LOG(screenCaptureService_ != nullptr, MSERR_NO_MEMORY,
        "screen capture service does not exist..");
    return screenCaptureService_->SetMicrophoneEnable(isMicrophone);
}
...
int32_t ScreenCaptureImpl::Init(AVScreenCaptureConfig config)
{
    ...
    switch (config.dataType) {
        case ORIGINAL_STREAM: {
            ...
            CHECK_AND_RETURN_RET_LOG(ret == MSERR_OK, MSERR_INVALID_VAL, "initMicAudioCap failed");
            ret = screenCaptureService_->InitVideoCap(config.videoInfo.videoCapInfo);
            CHECK_AND_RETURN_RET_LOG(ret == MSERR_OK, MSERR_INVALID_VAL, "initVideoCap failed");
            ...
            if (NeedStartInnerAudio(config.audioInfo.innerCapInfo.audioSource)) {
                ...
                CHECK_AND_RETURN_RET_LOG(ret == MSERR_OK, MSERR_INVALID_VAL, "initInnerAudioCap failed");
            }
            break;
        }
        case ENCODED_STREAM:
            // start init audioenc && videoenc
...
int32_t ScreenCaptureImpl::StartScreenCapture(DataType type)
{
    ...
    switch (type) {
        case ORIGINAL_STREAM:
            MEDIA_LOGI("start cap original stream");
            break;
        case ENCODED_STREAM:
            MEDIA_LOGI("start cap encoded stream,still not support");
            return MSERR_UNSUPPORT;
...
int32_t ScreenCaptureImpl::AcquireAudioBuffer(std::shared_ptr<AudioBuffer> &audiobuffer, AudioCapSourceType type)
{
...
sptr<OHOS::SurfaceBuffer> ScreenCaptureImpl::AcquireVideoBuffer(int32_t &fence, int64_t &timestamp, OHOS::Rect &damage)
{
    ...
    // void *buffer = videobuffer->GetVirAddr();
    if (ret != MSERR_OK) {
...
int32_t ScreenCaptureImpl::ReleaseAudioBuffer(AudioCapSourceType type)
{
