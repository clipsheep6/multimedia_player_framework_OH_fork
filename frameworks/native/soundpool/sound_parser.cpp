...
int32_t SoundParser::DoDecode(MediaAVCodec::Format trackFormat)
{
    ...
    if (trackTypeInfo == MEDIA_TYPE_AUD) {
        ...
        if (AUDIO_RAW_MIMETYPE_INFO.compare(trackMimeTypeInfo) == 0) {
            ...
            audioDec_ = MediaAVCodec::AudioDecoderFactory::CreateByName((AVCodecCodecName::AUDIO_DECODER_AAC_NAME).data());
        } else {
...
void SoundDecoderCallback::OnInputBufferAvailable(uint32_t index, std::shared_ptr<AVSharedMemory> buffer)
{
    ...
    if (AUDIO_RAW_MIMETYPE_INFO.compare(trackMimeTypeInfo_) == 0) {
        ...
        if (ret == 0) {
            ...
            MEDIA_ERR_LOG("SoundDecoderCallback OnInputBufferAvailable error parser:%{public}d", ret);
        }
        return;
    }
