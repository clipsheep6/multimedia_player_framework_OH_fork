...
uint8_t* OH_AV_MemoryGetAddr(AVMemory *mem)
{
...
int32_t OH_AV_MemoryGetSize(AVMemory *mem)
{
...
uint32_t OH_AV_MemoryGetFlags(AVMemory *mem)
{
    CHECK_AND_RETURN_RET_LOG(mem != nullptr, 0, "input mem is nullptr!");
    CHECK_AND_RETURN_RET_LOG(mem->magic_ == AVMagic::MEDIA_MAGIC_SHARED_MEMORY, 0, "magic error!");
    CHECK_AND_RETURN_RET_LOG(mem->memory_ != nullptr, 0, "memory is nullptr!");    
    return mem->memory_->GetFlags();
}