...
void RecorderCallbackNapi::OnInfo(int32_t type, int32_t extra)
{
    ...
    if (refMap_.find(INFO_CALLBACK_NAME) == refMap_.end()) {
        MEDIA_LOGW("can not find error callback!");
        return;
    }
    ...
    cb->autoRef = refMap_.at(INFO_CALLBACK_NAME);
    cb->callbackName = INFO_CALLBACK_NAME;
    cb->infoType = type;
    cb->infoExtra = extra;
...
void RecorderCallbackNapi::OnJsInfoCallBack(RecordJsCallback *jsCb) const
{
    uv_loop_s *loop = nullptr;
    napi_get_uv_event_loop(env_, &loop);
    if (loop == nullptr) {
    ...
    int ret = uv_queue_work(loop, work, [] (uv_work_t *work) {}, [] (uv_work_t *work, int status) {
        ...
        RecordJsCallback *event = reinterpret_cast<RecordJsCallback *>(work->data);
        std::string request = event->callbackName;
        MEDIA_LOGD("OnJsInfoCallBack %{public}s, uv_queue_work start", request.c_str());
        do {
            CHECK_AND_BREAK_LOG(status != UV_ECANCELED, "%{public}s canceled", request.c_str());
            std::shared_ptr<AutoRef> ref = event->autoRef.lock();
            CHECK_AND_BREAK_LOG(ref != nullptr, "%{public}s AutoRef is nullptr", request.c_str());
            ...
            nstatus = napi_create_int32(ref->env_, event->infoType, &args[0]);
            CHECK_AND_BREAK_LOG(nstatus == napi_ok && args[0] != nullptr, "%{public}s fail to get type value",
                request.c_str());

            nstatus = napi_create_int32(ref->env_, event->infoExtra, &args[1]);
            CHECK_AND_BREAK_LOG(nstatus == napi_ok && args[1] != nullptr, "%{public}s fail to get extra value",
                request.c_str());
