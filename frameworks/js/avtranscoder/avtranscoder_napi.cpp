...
// TO DO
RetInfo GetReturnInfo(int32_t errCode, const std::string &operate, const std::string &param, const std::string &add = "")
{
...
std::shared_ptr<TaskHandler<RetInfo>> AVTransCoderNapi::GetPrepareTask(std::unique_ptr<AVTransCoderAsyncContext> &asyncCtx)
{
    return std::make_shared<TaskHandler<RetInfo>>([napi = asyncCtx->napi, config = asyncCtx->config_]() {
        ...
        CHECK_AND_RETURN_RET(ret == MSERR_OK, ((void)napi->transCoder_->Cancel(), GetReturnInfo(ret, "Prepare", "")));
...
RetInfo AVTransCoderNapi::Start()
{
    ...
    CHECK_AND_RETURN_RET(ret == MSERR_OK, GetReturnInfo(ret, "Start", ""));
...
RetInfo AVTransCoderNapi::Pause()
{
    ...
    CHECK_AND_RETURN_RET(ret == MSERR_OK, GetReturnInfo(ret, "Pause", ""));
...
RetInfo AVTransCoderNapi::Resume()
{
    ...
    CHECK_AND_RETURN_RET(ret == MSERR_OK, GetReturnInfo(ret, "Resume", ""));
    StateCallback(AVTransCoderState::STATE_STARTED);
    return RetInfo(MSERR_EXT_API9_OK, "");
}
...
RetInfo AVTransCoderNapi::Cancel()
{
    ...
    CHECK_AND_RETURN_RET(ret == MSERR_OK, GetReturnInfo(ret, "Stop", ""));
...
{
    CHECK_AND_RETURN_RET(transCoder_ != nullptr, GetReturnInfo(MSERR_INVALID_OPERATION, "Configure", ""));
    CHECK_AND_RETURN_RET(config != nullptr, GetReturnInfo(MSERR_INVALID_VAL, "Configure", "config"));
    ...
    ret = transCoder_->SetAudioEncoder(config->audioCodecFormat);
        CHECK_AND_RETURN_RET(ret == MSERR_OK, GetReturnInfo(ret, "SetAudioEncoder", "audioCodecFormat"));
    
    ret = transCoder_->SetAudioEncodingBitRate(config->audioBitrate);
        CHECK_AND_RETURN_RET(ret == MSERR_OK, GetReturnInfo(ret, "SetAudioEncoderBitRate", "audioBitrate"));
    
    ret = transCoder_->SetVideoEncoder(config->videoCodecFormat);
        CHECK_AND_RETURN_RET(ret == MSERR_OK, GetReturnInfo(ret, "SetVideoEncoder", "videoCodecFormat"));
    
    ret = transCoder_->SetVideoEncodingBitRate(config->videoBitrate);
        CHECK_AND_RETURN_RET(ret == MSERR_OK, GetReturnInfo(ret, "SetVideoEncoderBitRate", "videoBitrate"));

    ret = transCoder_->SetVideoSize(config->videoFrameWidth, config->videoFrameHeight);
        CHECK_AND_RETURN_RET(ret == MSERR_OK, GetReturnInfo(ret, "SetVideoSize", "videoSize"));

    ret = transCoder_->SetOutputFormat(config->fileFormat);
        CHECK_AND_RETURN_RET(ret == MSERR_OK, GetReturnInfo(ret, "SetOutputFormat", "fileFormat"));

    ret = MSERR_INVALID_VAL;
    const std::string fdHead = "fd://";
    CHECK_AND_RETURN_RET(config->srcUrl.find(fdHead) != std::string::npos, GetReturnInfo(ret, "Getfd", "uri"));
    int32_t fd = -1;
    std::string inputFd = config->srcUrl.substr(fdHead.size());
    CHECK_AND_RETURN_RET(StrToInt(inputFd, fd) == true && fd >= 0, GetReturnInfo(ret, "Getfd", "uri"));

    ret = transCoder_->SetInputFile(fd);
    CHECK_AND_RETURN_RET(ret == MSERR_OK, GetReturnInfo(ret, "SetInputFile", "uri"));

    ret = MSERR_INVALID_VAL;
    CHECK_AND_RETURN_RET(config->dstUrl.find(fdHead) != std::string::npos, GetReturnInfo(ret, "Getfd", "uri"));
    fd = -1;
    std::string outputFd = config->dstUrl.substr(fdHead.size());
    CHECK_AND_RETURN_RET(StrToInt(outputFd, fd) == true && fd >= 0, GetReturnInfo(ret, "Getfd", "uri"));

    ret = transCoder_->SetOutputFile(fd);
    CHECK_AND_RETURN_RET(ret == MSERR_OK, GetReturnInfo(ret, "SetOutputFile", "uri"));
...
int32_t AVTransCoderNapi::GetAudioConfig(std::unique_ptr<AVTransCoderAsyncContext> &asyncCtx, napi_env env, napi_value args)
{
    ...
    CHECK_AND_RETURN_RET(CommonNapi::GetPropertyInt32(env, item, "audioBitrate", config->audioBitrate),
...
int32_t AVTransCoderNapi::GetVideoConfig(std::unique_ptr<AVTransCoderAsyncContext> &asyncCtx, napi_env env, napi_value args)
{
    ...
    CHECK_AND_RETURN_RET(CommonNapi::GetPropertyInt32(env, item, "videoBitrate", config->videoBitrate),
        (asyncCtx->AVTransCoderSignError(ret, "GetVideoBitrate", "videoBitrate"), ret));
    
    CHECK_AND_RETURN_RET(CommonNapi::GetPropertyInt32(env, item, "videoFrameWidth", config->videoFrameWidth),
        (asyncCtx->AVTransCoderSignError(ret, "GetVideoFrameWidth", "videoFrameWidth"), ret));
    
    CHECK_AND_RETURN_RET(CommonNapi::GetPropertyInt32(env, item, "videoFrameHeight", config->videoFrameHeight),
...
int32_t AVTransCoderNapi::GetConfig(std::unique_ptr<AVTransCoderAsyncContext> &asyncCtx, napi_env env, napi_value args)
{
    ...
    ret = AVTransCoderNapi::GetOutputFormat(outputFile, config->fileFormat);
    CHECK_AND_RETURN_RET(ret == MSERR_OK, (asyncCtx->AVTransCoderSignError(ret, "GetOutputFormat", "fileFormat"), ret));

    config->srcUrl = CommonNapi::GetPropertyString(env, args, "srcUrl");
    CHECK_AND_RETURN_RET(config->srcUrl != "",
        (asyncCtx->AVTransCoderSignError(MSERR_INVALID_VAL, "getSrcUrl", "srcUrl"), MSERR_INVALID_VAL));

    config->dstUrl = CommonNapi::GetPropertyString(env, args, "dstUrl");
    CHECK_AND_RETURN_RET(config->dstUrl != "",
        (asyncCtx->AVTransCoderSignError(MSERR_INVALID_VAL, "getDstUrl", "dstUrl"), MSERR_INVALID_VAL));
