...
private:
    ...
    static std::shared_ptr<TaskHandler<RetInfo>> GetInitTast(const std::unique_ptr<AVScreenCaptureAsyncContext> &asyncCtx);
    static std::shared_ptr<TaskHandler<RetInfo>> GetSetMicrophoneEnableTast(const std::unique_ptr<AVScreenCaptureAsyncContext> &asyncCtx,
        const bool enable);
