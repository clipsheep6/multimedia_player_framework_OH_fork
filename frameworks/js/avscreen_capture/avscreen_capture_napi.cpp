...
napi_value AVScreenCaptureNapi::JsInit(napi_env env, napi_callback_info info)
{
    ...
    if (asyncCtx->napi->GetConfig(asyncCtx, env, args[0]) == MSERR_OK) {
        asyncCtx->task_ = AVScreenCaptureNapi::GetInitTast(asyncCtx);
...
napi_value AVScreenCaptureNapi::JsSetMicrophoneEnabled(napi_env env, napi_callback_info info)
{
    ...
    asyncCtx->deferred = CommonNapi::CreatePromise(env, asyncCtx->callbackRef, result);
    asyncCtx->task_ = AVScreenCaptureNapi::GetSetMicrophoneEnableTast(asyncCtx, enable);
...
napi_value AVScreenCaptureNapi::JsSetEventCallback(napi_env env, napi_callback_info info)
{
    ...
    if (callbackName != AVScreenCaptureEvent::EVENT_ERROR && callbackName != AVScreenCaptureEvent::EVENT_STATE_CHANGE) {
        avScreenCaptureNapi->ErrorCallback(MSERR_INVALID_VAL, "SetEventCallback");
        return result;
    }
...
std::shared_ptr<TaskHandler<RetInfo>> AVScreenCaptureNapi::GetInitTast(
...
std::shared_ptr<TaskHandler<RetInfo>> AVScreenCaptureNapi::GetSetMicrophoneEnableTast(
    const std::unique_ptr<AVScreenCaptureAsyncContext> &asyncCtx, const bool enable)
{
    return std::make_shared<TaskHandler<RetInfo>>([napi = asyncCtx->napi, enable]() {
        ...
        CHECK_AND_RETURN_RET(ret == MSERR_OK, ((void)napi->screenCapture_->Release(), GetReturnInfo(ret, "SetMicrophoneEnable", "")));
    ...
    napi_env env, napi_callback_info info, size_t &argCount, napi_value *args)
{
    ...
    CHECK_AND_RETURN_RET_LOG(status == napi_ok && screenCaptureNapi != nullptr, nullptr, "failed to retrieve instance");
    ...
    napi_env env, napi_value args)
{
    ...
    CHECK_AND_RETURN_RET(ret == MSERR_OK, (asyncCtx->AVScreenCaptureSignError(ret, "getAudioSampleRate", "audioSampleRate"), ret));
    ...
    CHECK_AND_RETURN_RET(ret == MSERR_OK, (asyncCtx->AVScreenCaptureSignError(ret, "getAudioChannelCount", "audioChannelCount"), ret));
    ...
    CHECK_AND_RETURN_RET(ret == MSERR_OK, (asyncCtx->AVScreenCaptureSignError(ret, "getAudioBitrate", "audioBitrate"), ret));
    ...
    napi_env env, napi_value args)
{
    ...
    CHECK_AND_RETURN_RET(ret == MSERR_OK, (asyncCtx->AVScreenCaptureSignError(ret, "getVideoBitrate", "videoBitrate"), ret));
    ...
    CHECK_AND_RETURN_RET(ret == MSERR_OK, (asyncCtx->AVScreenCaptureSignError(ret, "getFrameWidth", "frameWidth"), ret));
    videoConfig.videoFrameWidth = frameWidth;
    ret = AVScreenCaptureNapi::GetPropertyInt32(env, args, "frameHeight", frameHeight);
    CHECK_AND_RETURN_RET(ret == MSERR_OK, (asyncCtx->AVScreenCaptureSignError(ret, "getFrameHeight", "frameHeight"), ret));
    videoConfig.videoFrameHeight = frameHeight;
    MEDIA_LOGI("input frameWidth %{public}d, frameHeight %{public}d", videoConfig.videoFrameWidth, videoConfig.videoFrameHeight);
