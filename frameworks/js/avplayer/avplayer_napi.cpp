...
napi_value AVPlayerNapi::JsAddSubtitleAVFileDescriptor(napi_env env, napi_callback_info info)
{
    ...
    if (status != napi_ok) {
        ...
        jsPlayer->OnErrorCb(MSERR_EXT_API9_INVALID_PARAMETER, "invalid parameters, please check JsAddSubtitleAVFileDescriptor");
        return result;
    }
    int64_t  offset = -1;
    ...
    auto task = std::make_shared<TaskHandler<void>>([jsPlayer,fd_s,offset,length]() {
        MEDIA_LOGE("AddSubtitleAVFileDescriptor Task");
        if (jsPlayer->player_ != nullptr) {
