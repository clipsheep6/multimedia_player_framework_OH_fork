...
const std::map<std::string, ContainerFormatType> FORMAT_TO_CTF = {
    {"mp4", CFT_MPEG_4},
    {"mpeg-ts", CFT_MPEG_TS},
    {"mkv", CFT_MKV},
    {"webm", CFT_WEBM},
...
const std::map<ContainerFormatType, OutputFormatType> CTP_TO_FORMAT = {
...
int32_t MapContainerFormat(const std::string &format, ContainerFormatType &cft)
{
    if (FORMAT_TO_CTF.count(format) != 0) {
        cft =  FORMAT_TO_CTF.at(format);
        return MSERR_OK;
    }
    return MSERR_INVALID_VAL;
}

int32_t MapOutputFormatType(const ContainerFormatType &cft, OutputFormatType &opf)
{
    if (CTP_TO_FORMAT.count(cft) != 0) {
        opf =  CTP_TO_FORMAT.at(cft);
        return MSERR_OK;
    }
    return MSERR_INVALID_VAL;
}

int32_t MapAudioCodec(const CodecMimeType &mime, AudioCodecFormat &audio)
{
...
int32_t MapVideoCodec(const CodecMimeType &mime, VideoCodecFormat &video)
{
