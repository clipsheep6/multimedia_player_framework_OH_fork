...
namespace OHOS {
namespace Media {
std::shared_ptr<AudioEncoder> AudioEncoderFactory::CreateByCodecName(const std::string &name)
{
...
std::shared_ptr<AudioEncoder> AudioEncoderFactory::CreateByEncoderName(const std::string &name)
{
    std::shared_ptr<AudioEncoderImpl> impl = std::make_shared<AudioEncoderImpl>();
    CHECK_AND_RETURN_RET_LOG(impl != nullptr, nullptr, "failed to new AudioEncoderImpl");
...
{
    CHECK_AND_RETURN_RET_LOG(codecService_ != nullptr, MSERR_INVALID_OPERATION, "service died");
    return codecService_->QueueInputBuffer(index, info, AVCODEC_BUFFER_TYPE_NONE, flag);
}
