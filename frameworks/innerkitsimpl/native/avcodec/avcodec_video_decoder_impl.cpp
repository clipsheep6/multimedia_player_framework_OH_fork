...
namespace OHOS {
namespace Media {
std::shared_ptr<VideoDecoder> VideoDecoderFactory::CreateByCodecName(const std::string &name)
{
...
std::shared_ptr<VideoDecoder> VideoDecoderFactory::CreateByDecoderName(const std::string &name)
{
    std::shared_ptr<VideoDecoderImpl> impl = std::make_shared<VideoDecoderImpl>();
    CHECK_AND_RETURN_RET_LOG(impl != nullptr, nullptr, "failed to new VideoDecoderImpl");
...
int32_t VideoDecoderImpl::QueueInputBuffer(uint32_t index, AVCodecBufferInfo info,
                                           AVCodecBufferType type, AVCodecBufferFlag flag)
{
    CHECK_AND_RETURN_RET_LOG(codecService_ != nullptr, MSERR_INVALID_OPERATION, "service died");
    return codecService_->QueueInputBuffer(index, info, type, flag);
}
