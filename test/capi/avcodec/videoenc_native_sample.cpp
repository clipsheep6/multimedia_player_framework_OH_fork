...
void VEncNdkSample::RunVideoEnc(void)
{
    ...
    NATIVE_CHECK_AND_RETURN_LOG(GetSurface() == AV_ERR_OK && surface_ != nullptr, "Fatal: GetSurface fail");
...
void VEncNdkSample::GenerateData(uint32_t count, uint32_t fps)
{
    ...
    while (frameCount_ <= count) {
        ...
        NATIVE_CHECK_AND_BREAK_LOG(buffer != nullptr, "Fatal: SurfaceBuffer is nullptr");
        ...
        NATIVE_CHECK_AND_BREAK_LOG(memset_s(addr, buffer->GetSize(), 0xFF, YUV_BUFFER_SIZE) == EOK, "Fatal"); // 纯白色0xff

        const sptr<OHOS::BufferExtraData>& extraData = buffer->GetExtraData();
        NATIVE_CHECK_AND_BREAK_LOG(extraData != nullptr, "Fatal: SurfaceBuffer is nullptr");
...
struct AVCodec* VEncNdkSample::CreateVideoEncoder(void)
{
    ...
    cb_.onError = VencAsyncError;
    cb_.onStreamChanged = VencAsyncStreamChanged;
    cb_.onNeedInputData = VencAsyncNeedInputData;
    cb_.onNeedOutputData = VencAsyncNewOutputData;
...
int32_t VEncNdkSample::GetSurface()
{
    ...
    return AV_ERR_OK;
    // return OH_VideoEncoder_GetSurface(venc_, window);
}
