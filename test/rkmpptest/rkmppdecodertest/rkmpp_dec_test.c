 ...
 * Author：Jack Fan
 * Date：2022-08-11
 * Function: rkmpp decoder test example
...
static int dec_simple(MpiDecLoopData *data)
{
...
#else
    {
        ...
        {
            char path[500];
            snprintf(path,sizeof(path),"/data/test4_h264_/%d.h264",index);
            printf("open %s\n",path);
            h264File = fopen(path,"rb");
        }
        index ++;
        ...
        int datalen = fread(data->frm_buf,1,data->frm_buf_len,h264File );
        fclose(h264File);
        if( datalen < 1 )
        {
            data->loop_end = 1;
        ...
        mpp_packet_set_data(packet, data->frm_buf);
        mpp_packet_set_size(packet, datalen);
        mpp_packet_set_pos(packet, data->frm_buf);
        mpp_packet_set_length(packet, datalen);   
    }
    ...
    {
        char path[500];
        snprintf(path,sizeof(path),"/data/test4_h264/%d.h264",index);
        index ++;
        h264File = fopen(path,"wb");
    }
    if( h264File != NULL)
    {
        fwrite(slot->data,slot->size,1,h264File);
        fclose(h264File);
    }
#endif
    ...
    {
        int total = mpi->control(ctx,MPP_DEC_GET_TOTAL_PACKET_SLOTS,NULL);
        int used = mpi->control(ctx,MPP_DEC_GET_USED_PACKET_SLOTS,NULL);
        int freed = mpi->control(ctx,MPP_DEC_GET_FREE_PACKET_SLOTS,NULL);

        //printf("total=%d,used=%d,freed=%d\n",total,used,freed);
    }
    // setup eos flag
    if (pkt_eos)
...
int dec_decode(MpiDecTestCmd *cmd)
{
    ...
    data.frm_buf_len = 1920*1080*4;
    data.frm_buf = (char *)malloc(data.frm_buf_len);
