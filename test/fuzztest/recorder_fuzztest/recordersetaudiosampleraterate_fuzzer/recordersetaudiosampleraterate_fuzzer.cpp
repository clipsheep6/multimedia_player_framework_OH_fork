...
#include "recordersetaudiosampleraterate_fuzzer.h"
#include <cmath>
#include <iostream>
#include "aw_common.h"
#include "string_ex.h"
...
namespace OHOS {
namespace Media {
RecorderSetAudioSampleRateRateFuzzer::RecorderSetAudioSampleRateRateFuzzer()
{
}
...
bool RecorderSetAudioSampleRateRateFuzzer::FuzzRecorderSetAudioSampleRateRate(uint8_t *data, size_t size)
{
    ...
    g_videoRecorderConfig.outputFd = open("/data/test/media/recorder_SetAudioSampleRateRate.mp4", O_RDWR);
    ...
    if (g_videoRecorderConfig.outputFd >= 0) {
        ...
        g_videoRecorderConfig.sampleRate = *reinterpret_cast<int32_t *>(data);
...
bool FuzzTestRecorderSetAudioSampleRateRate(uint8_t *data, size_t size)
{
    if ((data == nullptr) || (size < sizeof(int32_t))) {
        return true;
    }
    RecorderSetAudioSampleRateRateFuzzer testRecorder;
    return testRecorder.FuzzRecorderSetAudioSampleRateRate(data, size);
}
}
...
extern "C" int LLVMFuzzerTestOneInput(uint8_t *data, size_t size)
{
    ...
    OHOS::FuzzTestRecorderSetAudioSampleRateRate(data, size);
    return 0;
}