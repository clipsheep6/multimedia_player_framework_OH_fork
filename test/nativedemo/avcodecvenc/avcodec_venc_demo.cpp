...
void VEncDemo::RunCase(bool enableProp)
{
    ...
    DEMO_CHECK_AND_RETURN_LOG(SetParameter(parameter) == MSERR_OK, "Fatal: SetParameter fail");
...
void VEncDemo::GenerateData(uint32_t count, uint32_t fps)
{
    ...
    while (frameCount <= count) {
        ...
        frameCount++;
        (frameCount % 30) == 0 ? (isKeyFrame_ = 1) : (isKeyFrame_ = 0); // 30 : I frame every 30 frames
    }
}

int32_t VEncDemo::CreateVenc() noexcept
{
    string encodeMode = "";
