...
describe('AVRecorderStabilityTest', function () {
    ...
    async function getInputSurfaceByPromise() {
    ...
    function setCallbackOn1() {
        console.info('case callback on 1');
        avRecorder.on('stateChange', async (state, reason) => {
            console.info('case state 1 has changed, new state is :' + state);
            })
        avRecorder.on('error', (err) => {
            console.info('case avRecorder.on(error) 1 called, errMessage is ' + err.message);
        });
    }

    function setCallbackOn2() {
        console.info('case callback on 2');
        avRecorder.on('stateChange', async (state, reason) => {
            console.info('case state 2 has changed, new state is :' + state + ',and new reason is : ' + reason);
            })
        avRecorder.on('error', (err) => {
            console.info('case avRecorder.on(error) 2 called, errMessage is ' + err.message);
        });
    }

    function setCallbackOn3() {
        console.info('case callback on 3');
        avRecorder.on('stateChange', async (state, reason) => {
            console.info('case state 3 has changed, new state is :' + state + ',and new reason is : ' + reason);
            })
        avRecorder.on('error', (err) => {
            console.info('case avRecorder.on(error) 3 called, errMessage is ' + err.message);
        });
    }

    function setCallbackOff() {
    ...
    it('Sub_Multimedia_Media_AVRecorder_On_Off_Promise', 0, async function (done) {
