...
export default function AVRecorderDoubleCallTest() {
describe('AVRecorderDoubleCallTest', function () {
    ...
    /* *
    * @tc.number    : Sub_Multimedia_Media_AudioRecorder_Getsurface_0100
    ...
    * @tc.level     : Level0
    */
    it('Sub_Multimedia_Media_AudioRecorder_Getsurface_0100', 0, async function (done) {
        avConfig = audioConfig;
        caseCount = 1;
        let fileName = 'audioRecorder_audioGetsurface_0'+ caseCount +'.mp4';
        await getRecorderFileFd(fileName, 'video');

        console.info('case to createAVRecorder promise');
        await media.createAVRecorder().then((recorder) => {
            console.info('case createAVRecorder promise called');
            if (typeof (recorder) != 'undefined') {
                avRecorder = recorder;
        ...
        await avRecorder.prepare(avConfig).then(() => {
            console.info('case recorder prepare by promise 1 called');
        }, mediaTestBase.failureCallback).catch(mediaTestBase.catchCallback);
    
        console.info('case to getInputSurface promise 1');
        await avRecorder.getInputSurface().then((outputSurface) => {
            console.info('case getInputSurface by promise 1 called');
            surfaceID = outputSurface;
            console.info('case outputSurface surfaceID is: ' + surfaceID);
        }, printFailure).catch(printCatchError);
        console.info('case to start promise 1');
        await avRecorder.start().then(() => {
            console.info('case recorder start by promise 1 called');
        }, mediaTestBase.failureCallback).catch(mediaTestBase.catchCallback);
